...
public class SeraphCommands {
        ...
        dispatcher.register(CommandManager.literal("angeldeal").requires(serverCommandSource -> serverCommandSource.hasPermissionLevel(3)).then(CommandManager.literal("get").then(CommandManager.argument("player", EntityArgumentType.player()).executes(context -> {
            ...
            if (!angelDealAccessor.getAngelDeals().isEmpty()) {
                ...
                for (AngelDeal.Instance instance : angelDealAccessor.getAngelDeals()) {
                    angelDeals.append(SeraphRegisters.ANGEL_DEALS.getId(instance.angelDeal).toString()).append(", ");
                }
            ...
            if (angelDealAccessor.hasAngelDeal(angelDeal)) {
                ...
                context.getSource().sendFeedback(new TranslatableText("commands.angeldeal.get.single", player.getEntityName(), days, SeraphRegisters.ANGEL_DEALS.getId(angelDeal)), false);
                return Command.SINGLE_SUCCESS;
            }
            ...
            context.getSource().sendFeedback(new TranslatableText("commands.angeldeal.add", player.getEntityName(), 7, SeraphRegisters.ANGEL_DEALS.getId(angelDeal)), true);
            return Command.SINGLE_SUCCESS;
            ...
            context.getSource().sendFeedback(new TranslatableText("commands.angeldeal.add", player.getEntityName(), days, SeraphRegisters.ANGEL_DEALS.getId(angelDeal)), true);
            return Command.SINGLE_SUCCESS;
            ...
            if (angelDealAccessor.hasAngelDeal(angelDeal)) {
                ...
                context.getSource().sendFeedback(new TranslatableText("commands.angeldeal.remove", player.getEntityName(), SeraphRegisters.ANGEL_DEALS.getId(angelDeal)), true);
                return Command.SINGLE_SUCCESS;
            }
    ...
    private static class AngelDealArgumentType implements ArgumentType<AngelDeal> {
        ...
        public AngelDeal parse(StringReader reader) throws CommandSyntaxException {
            ...
            return SeraphRegisters.ANGEL_DEALS.getOrEmpty(identifier).orElseThrow(() -> INVALID_ANGEL_DEAL_EXCEPTION.create(identifier));
        }

        @Override
        public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
            return CommandSource.suggestIdentifiers(SeraphRegisters.ANGEL_DEALS.getIds(), builder);
        }
    }
